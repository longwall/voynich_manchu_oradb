CREATE OR REPLACE PACKAGE BODY metaphone_pkg
as
 ...
 *  Version:   1.01
 *   
 *  Bugfixes (9 fixes; v1.01) By Edwin Korsten, http://www.oradev.com
 *
...
is
begin
  if p_string <> metaphone_pkg.input_string 
  or metaphone_pkg.input_string is null
  or p_keylen <> metaphone_pkg.keylen then
    begin
      metaphone_pkg.input_string := p_string;
      metaphone_pkg.keylen := p_keylen;
      generate(p_string, p_keylen, metaphone_pkg.primkey, metaphone_pkg.altkey);
    end;
  end if;
  return metaphone_pkg.primkey;
end genprimkey;
...
is
   ...
   begin
     ...
     -- Bugfix: when achar is space, do not add to alt-key
     if length(l_achar) > 0 or length(achar)>0 then 
...
--                  !!! WHY NO INCREMENT HERE???  L_POS := L_POS + 2;
           ...
           -- Bugfix. Position starts at 1
           and stringAt(l_word, 1, 6) not in ('DANGER','RANGER','MANGER')
             ...
             begin
               if charAt(l_word, l_pos + 2) = 'H' then
                 if stringAt(l_word, l_pos+3, 2) in ('OO','ER','EN','UY','ED','EM') then
                   ...
                   begin
                     -- bugfix check on 4th character W
                     if l_pos = 1 and not isVowel(charAt(l_word, 4)) and charat(l_word,4) <> 'W' then 
                       addMetaphoneCharacter('X','S');
                     else
                       addMetaphoneCharacter('X');
                     end if;
       ...
       when charAt(l_word, l_pos) = 'X' then
         begin
           if not (l_pos = l_len and
                    -- bugfix
                    (nvl(stringAt(l_word, l_pos - 3, 3),'X') in ('IAU','EAU') or 
